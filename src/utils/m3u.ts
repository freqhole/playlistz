...
export function parseM3U(content: string): M3UPlaylist {
  ...
  for (const line of lines) {
    ...
    if (t.startsWith("# Playlist:"))
      result.title = t.slice("# Playlist:".length).trim();
    else if (t.startsWith("# Description:"))
      result.description = t.slice("# Description:".length).trim();
    else if (t.startsWith("# PlaylistImage:"))
      result.playlistImageFile = t.slice("# PlaylistImage:".length).trim();
    else if (t.startsWith("# PlaylistId:"))
      result.id = t.slice("# PlaylistId:".length).trim();
    else if (t.startsWith("# PlaylistRev:"))
      result.rev = parseInt(t.slice("# PlaylistRev:".length).trim(), 10);
      ...
      pendingTitle = t.slice("# Title:".length).trim();
    else if (t.startsWith("# Artist:"))
      pendingArtist = t.slice("# Artist:".length).trim();
    else if (t.startsWith("# Album:"))
      pendingAlbum = t.slice("# Album:".length).trim();
    else if (t.startsWith("# Image:"))
      pendingImage = t.slice("# Image:".length).trim();
